...
export interface RealUploadSectionRef {
  setFromSample: (imageUrl: string, name: string, category: string) => void;
}

const RealUploadSection = forwardRef<RealUploadSectionRef, RealUploadSectionProps>(
  ({ onProjectCreated }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      setFromSample: (imageUrl: string, name: string, cat: string) => {
        setSampleUrl(imageUrl);
        setPreview(imageUrl);
        setProjectName(name);
        setCategory(cat);
        setFile(null);
    ...
    const handleUpload = async () => {
      ...
      try {
        ...
        setSuccess(true);
        onProjectCreated({ ...projectData, status: 'processing' });
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select value={provider} onValueChange={setProvider}>
                    ...
                    <SelectContent>
                      <SelectItem value="instantmesh">InstantMesh (Fast)</SelectItem>
                      <SelectItem value="trellis">TRELLIS (Microsoft)</SelectItem>
                      <SelectItem value="triposr">TripoSR (Stability AI)</SelectItem>
                      <SelectItem value="wonder3d">Wonder3D (High Detail)</SelectItem>
                      <SelectItem value="meshy">Meshy.ai</SelectItem>
                      <SelectItem value="tripo3d">Tripo3D</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-gray-500 text-xs mt-2 flex items-center gap-1">
                  ...
                  Or select a sample image from above to test instantly
                </p>
              </div>
              ...
              {success && (
                <div className="flex items-center gap-2 text-green-500 bg-green-500/10 p-4 rounded-lg">
                  ...
                  <span>Upload successful! 3D generation started...</span>
                </div>
              )}

              <Button
                onClick={handleUpload}
                disabled={(!file && !sampleUrl) || uploading}
              ...
              >
                {uploading ? (
                  ...
                  <>
                    ...
                    Generate 3D Model & Revit Family
                  </>
                )}
              </Button>
            </div>
...
RealUploadSection.displayName = 'RealUploadSection';

export default RealUploadSection;
